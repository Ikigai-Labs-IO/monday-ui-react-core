...
However, the import statement `{Heading}` from '@ikigailabs/origami-design' is already reserved for the <StorybookLink page="Text/LegacyHeading [deprecated]">LegacyHeading</StorybookLink> component.
Therefore, the new Heading with the updated API can be accessed using the following import statement:

```javascript
import { Heading } from "@ikigailabs/origami-design/next";
```
